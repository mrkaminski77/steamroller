...
### `drill_and_flatten(df, path_string, delimiter="__", literal_dot_replacement="_")`
...
- `path_string` — Dot-notation path to the target column (e.g. `"customer.address"`)
...
### `prune_to_relationship(df, target_path, delimiter="__", literal_dot_replacement="_")`
...
**Parameters:**
- `df` — PySpark DataFrame
- `target_path` — Dot-notation path identifying the relationship to keep (e.g. `"order.items"`)
...
### `add_metadata(df, path_string, run_id)`
...
| `_target_grain` | Provided `path_string` literal |
| `_record_hash` | `xxhash64` over all columns |

**Parameters:**
- `df` — PySpark DataFrame
- `path_string` — Path label to store as the target grain identifier
...
- `schema` — Optional `StructType` defining the expected schema. If provided, Spark skips inference (faster reads, stable types) and drift detection is enabled. If omitted, schema is inferred from the data.
